package com.zjwam.qualification.custom;
...
import com.zjwam.qualification.adapter.CurriculumGridViewAdapter;
import com.zjwam.qualification.bean.ClassSearchBean;
import com.zjwam.qualification.bean.CurriculumSelectBean;
...
public class CurriculumPopupWindow extends PopupWindow {
    ...
    private List<ClassSearchBean> classSearchBean;
    ...
    public CurriculumPopupWindow(Context context, List<ClassSearchBean> classSearchBean) {
    ...
    private void initData() {
        if (classSearchBean.size() > 0) {
            for (int i = 0; i < classSearchBean.size(); i++) {
                List<CurriculumSelectBean> list = new ArrayList<>();
                List<List<CurriculumSelectBean>> list_m = new ArrayList<>();
                if (!TextUtils.isEmpty(classSearchBean.get(i).getName())) {
                    if (classSearchBean.get(i).getCont().size() > 0) {
                        for (int d = 0; d < classSearchBean.get(i).getCont().size(); d++) {
                            List<CurriculumSelectBean> list_a = new ArrayList<>();//6
                            CurriculumSelectBean curriculumSelectBean = new CurriculumSelectBean();
                            curriculumSelectBean.setName(classSearchBean.get(i).getCont().get(d).getName());
                            curriculumSelectBean.setId(classSearchBean.get(i).getCont().get(d).getId());
                            list.add(curriculumSelectBean);
                            if (!TextUtils.isEmpty(classSearchBean.get(i).getCont().get(d).getName())) {
                                if (classSearchBean.get(i).getCont().get(d).getCont().size() > 0) {
                                    for (int a = 0; a < classSearchBean.get(i).getCont().get(d).getCont().size(); a++) {
                                        CurriculumSelectBean beans = new CurriculumSelectBean();
                                        beans.setName(classSearchBean.get(i).getCont().get(d).getCont().get(a).getName());
                                        beans.setId(classSearchBean.get(i).getCont().get(d).getCont().get(a).getId());
                ...
                CurriculumSelectBean curriculumSelectBean = new CurriculumSelectBean();
                curriculumSelectBean.setName(classSearchBean.get(i).getName());
                curriculumSelectBean.setId(classSearchBean.get(i).getId());
