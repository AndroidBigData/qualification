package com.zjwam.qualification.receiver;
...
import com.zjwam.qualification.utils.AppHelper;
import com.zjwam.qualification.utils.QlftPreference;
import com.zjwam.qualification.view.activity.LogoutNotifiActivity;
import com.zjwam.qualification.view.activity.NewsWebActivity;
        ...
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
            ...
            if (AppHelper.isAppAlive(context,context.getPackageName()) == 0){
                ...
                try {
                    ...
                    if (object.toString().contains("url") && QlftPreference.getInstance(context).IsFlag()) {
                        Bundle bundle1 = new Bundle();
                        bundle1.putString("url", object.getString("url"));
                        Intent i = new Intent(context, NewsWebActivity.class).putExtras(bundle1);
                        i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        context.startActivity(i);
                    } else if (object.toString().contains("id") && QlftPreference.getInstance(context).IsFlag()) {
                        Bundle bundle1 = new Bundle();
                        bundle1.putLong("id", Long.parseLong(object.getString("id")));
                        Intent i = new Intent(context, VideoPlayerActivity.class).putExtras(bundle1);
                        i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        context.startActivity(i);
                    }
